...
export class BannerTypesListPage implements OnInit, OnDestroy {
	...
	ngOnDestroy(): void {
		//console.warn('Banner Types List ngOnDestroy');
	...
	public deleteModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			if (result === true) {
				...
				/**/
				this.bannerTypeService.delete(id)
					.pipe(first())
					.subscribe({
						next: () => {
	...
	public restoreModel(id: string): void {
		...
		confirmDialog.afterClosed().subscribe(result => {
			if (result === true) {
				...
				/**/
				this.bannerTypeService.restore(id)
					.pipe(first())
					.subscribe({
						next: () => {
			...
			steps: [
				{
					...
					intro: "Add a new creative type."
				},
				{
					element: '#hint-action-btn-export',
					intro: "Click here to download a spreadsheet with a list of creative types."
				},
				{
				...
				},
				{
					...
					intro: "Enable creative type to make it available for designing or editing in dashboard or disable to hide it."
				},
				{
					element: '#btn-edit',
					intro: "Click here to edit creative type details."
				},
				{
					element: '#btn-audit',
					intro: "Click here to see creative type history."
				},
				{
					element: '#btn-delete',
					intro: "Click here to delete creative type."
				},
			]
			/**/
		});
	...
	public export(): void {
		...
		const exportArray = this.primaryData.map( (data, index) => {
			...
			return {
				...
				'No. of Creatives': data.banners.length,
		...
		/* save to file */
		XLSX.writeFile(wb, 'BAPP_Available_Creative_Types.xlsx');
