...
const fonttypeService = require(path.join(__dirname, '../services/fonttype.service'));
...
function getAll(req, res, next) {
	fonttypeService.getAll(req.user.role)
...
function getById(req, res, next) {
	...
	if ( Number(req.params.id) ) {
        fonttypeService.getById(req.params.id)
...
function createSchema(req, res, next) {
	const schema = Joi.object({
		...
		styleSheet: Joi.string().required()
    });
	validateRequest(req, next, schema);
}

function create(req, res, next) {
	fonttypeService.create(req.body, req.user.id)
...
function update(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		fonttypeService.update(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function updateStatus(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		fonttypeService.updateStatus(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function _delete(req, res, next) {
    ...
    if ( Number(req.params.id) ) {
		fonttypeService.delete(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function restore(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		fonttypeService.restore(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
