...
const taskService = require(path.join(__dirname, '../services/task.service'));
...
function getAll(req, res, next) {
	taskService.getAll(req.user.role)
...
function updateStatus(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		taskService.updateStatus(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function restore(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		taskService.restore(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
