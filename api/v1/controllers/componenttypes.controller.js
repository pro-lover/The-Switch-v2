...
const componenttypeService = require(path.join(__dirname, '../services/componenttype.service'));
...
function getAll(req, res, next) {
	componenttypeService.getAll(req.user.role)
...
function getById(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		componenttypeService.getById(req.params.id)
...
function create(req, res, next) {
	componenttypeService.create(req.body, req.user.id)
...
function update(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		componenttypeService.update(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function updateStatus(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		componenttypeService.updateStatus(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function _delete(req, res, next) {
    ...
    if ( Number(req.params.id) ) {
        componenttypeService.delete(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function restore(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		componenttypeService.restore(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
