...
const taxonomiesService = require(path.join(__dirname, '../services/taxonomy.service'));
...
function getAll(req, res, next) {
	taxonomiesService.getAll(req.user.role)
...
function getById(req, res, next) {
	...
	if ( Number(req.params.id) ) {
        taxonomiesService.getById(req.params.id)
...
function create(req, res, next) {
	taxonomiesService.create(req.body, req.user.id)
...
function update(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		taxonomiesService.update(req.params.id, req.body, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function _delete(req, res, next) {
    ...
    if ( Number(req.params.id) ) {
		taxonomiesService.delete(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
    } else {
		return res.status(401).json({ message: 'Unauthorized' });
...
function restore(req, res, next) {
	...
	if ( Number(req.params.id) ) {
		taxonomiesService.restore(req.params.id, req.user.id)
			.then(model => res.json(model))
			.catch(next);
	} else {
		return res.status(401).json({ message: 'Unauthorized' });
