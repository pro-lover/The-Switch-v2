...
async function getAll(reqRole) {
    ...
    const models = await db.EasingType.findAll({
...
async function getById(id) {
    const model = await getEasingType(id);
...
async function create(params, editId) {
	// validate
	if (await db.EasingType.findOne({ where: {
		[Op.or]: [{name: params.name}, {description: params.description}]
		} }))
	{
		throw 'Easing Type already exists. Please provide a unique Name and description.';
	}

    const model = new db.EasingType(params);
	model.lastEditedBy = editId;
    // save model
    await model.save();

    return basicDetails(await getEasingType(model.id));
}
...
async function update(id, params, editId) {
	...
	if (params.name && model.name == params.name && model.description == params.description ) {
		throw 'Easing Type hasn\'t been updated.';
	}
	// validate (if name/shortname is unique in db)
	if ( params.name && (model.name !== params.name) && await db.EasingType.findOne({ where: { name: params.name } })) {
		throw 'Easing Type already exists.';
	}
...
async function getEasingType(id) {
    ...
    const model = await db.EasingType.findByPk(id, {
		paranoid: false,
		/** /
		include:[
			{
    ...
    if (!model) throw 'Easing Type not found';
    return model;
}
...
async function getHistory() {
	...
	const modelHistories = await db.sequelizeInstance.query(
		"SELECT * FROM `easingtypeHistories`",
		{
...
async function getHistoryById(id) {
	...
	const modelHistories = await db.sequelizeInstance.query(
		"SELECT * FROM `easingtypeHistories` WHERE id = ?",
		{
			replacements: [id],
			type: db.sequelizeInstance.QueryTypes.SELECT
		}
