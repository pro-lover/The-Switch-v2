...
async function getAll(reqRole) {
    ...
    const models = await db.BannerSize.findAll({
...
async function getById(id) {
    const model = await getBannerSize(id);
...
async function create(params, editId) {
	// validate
	if (await db.BannerSize.findOne({ where: {
		[Op.or]: [{name: params.name}, {description: params.description}]
		} }))
	{
		throw 'Banner Size already exists. Please provide a unique Name and description.';
	}

    const model = new db.BannerSize(params);
	model.lastEditedBy = editId;
    // save model
    await model.save();

    return basicDetails(await getBannerSize(model.id));
}
...
async function update(id, params, editId) {
	...
	if (params.name && model.name == params.name && model.description == params.description ) {
		throw 'Banner Size hasn\'t been updated.';
	}
	// validate (if name/shortname is unique in db)
	if ( params.name && (model.name !== params.name) && await db.BannerSize.findOne({ where: { name: params.name } })) {
		throw 'Banner Size already exists.';
	}
...
async function getBannerSize(id) {
    ...
    const model = await db.BannerSize.findByPk(id, {
		paranoid: false,
		include:[
			{
				model: db.Banner,
    ...
    if (!model) throw 'Banner Size not found';
    return model;
}
...
async function getHistory() {
	...
	const modelHistories = await db.sequelizeInstance.query(
		"SELECT * FROM `bannersizeHistories`",
		{
...
async function getHistoryById(id) {
	...
	const modelHistories = await db.sequelizeInstance.query(
		"SELECT * FROM `bannersizeHistories` WHERE id = ?",
		{
			replacements: [id],
			type: db.sequelizeInstance.QueryTypes.SELECT
		}
...
function basicDetails(model) {
    const { id, name, description, status, width, height, created, updated, deletedAt, history, version, lastEditedBy, banners } = model;
    return { id, name, description, status, width, height, created, updated, deletedAt, history, version, lastEditedBy, banners };
}
