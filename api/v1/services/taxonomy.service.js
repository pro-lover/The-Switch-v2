...
async function getAll(reqRole) {
    ...
    const models = await db.Taxonomy.findAll({
...
async function getById(id) {
    const model = await getTaxonomy(id);
...
async function create(params, editId) {
	// validate
	if (await db.Taxonomy.findOne({ where: {
		[Op.or]: [{name: params.name}, {description: params.description}]
		} }))
	{
		throw 'Taxonomy already exists. Please provide a unique Name and description.';
	}

    const model = new db.Taxonomy(params);
	model.lastEditedBy = editId;
    // save model
    await model.save();

    return basicDetails(await getTaxonomy(model.id));
}
...
async function update(id, params, editId) {
	...
	if (params.name && model.name == params.name && model.description == params.description ) {
		throw 'Taxonomy hasn\'t been updated.';
	}
	// validate (if name/shortname is unique in db)
	if ( params.name && (model.name !== params.name) && await db.Taxonomy.findOne({ where: { name: params.name } })) {
		throw 'Taxonomy already exists.';
	}
...
async function getTaxonomy(id) {
    ...
    const model = await db.Taxonomy.findByPk(id, {
		paranoid: false,
		include:[
			//{
			//	model: db.Banner,
    ...
    if (!model) throw 'Taxonomy not found';
    return model;
}
...
async function getHistoryById(id) {
	...
	const modelHistories = await db.sequelizeInstance.query(
		"SELECT * FROM `taxonomyHistories` WHERE id = ?",
		{
			replacements: [id],
			type: db.sequelizeInstance.QueryTypes.SELECT
		}
